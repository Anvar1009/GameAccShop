...
export function BuyerOrdersPage() {
  ...
  if (isLoading) return <PageLoader label={t("buyerOrders.loading")} />;

  const list = [...(orders ?? [])].sort((a, b) => b.orderId - a.orderId);

  return (
    <div className="page-container">
      <PageHeader title={t("buyerOrders.title")} description={t("buyerOrders.desc")} />

      {isError ? (
        <ErrorState onRetry={() => refetch()} />
      ) : list.length === 0 ? (
        <EmptyState
          title={t("buyerOrders.noneTitle")}
          description={t("buyerOrders.noneDesc")}
          icon={ShoppingBag}
          action={
            <Button asChild>
                ...
                <TableRow key={o.orderId}>
                  ...
                  <TableCell className="text-right">
                    <Button asChild variant="outline" size="sm">
                      <Link to={`/orders/${o.orderId}`}>{t("common.details")}</Link>
                    </Button>
                  </TableCell>
                </TableRow>
              ))}
